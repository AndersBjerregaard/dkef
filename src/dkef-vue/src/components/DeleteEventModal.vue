...
async function deleteEvent() {
  ...
  try {
    await eventStore.deleteEventItem(props.event.id)
    await router.push({ name: 'events-and-news' })
    emit('close')
  } catch (err: unknown) {
    const errorMessage = err instanceof Error ? err.message : 'Kunne ikke slette arrangement'
...
<template>
  <BaseModal
    ...
    @close="emit('close')"
  >
    <p>Er du sikker på at du vil slette dette arrangement? Denne handling kan ikke fortrydes.</p>
