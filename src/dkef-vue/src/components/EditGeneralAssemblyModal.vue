...
const props = defineProps<{
  isOpen: boolean
  assembly: PublishedGeneralAssembly
}>()
...
function populateFields() {
  itemTitle.value = props.assembly.title
  itemSection.value = props.assembly.section
  itemAddress.value = props.assembly.address
  ...
  itemAttachments.value = toAttachmentItems(
    props.assembly.attachmentUrls,
    props.assembly.attachmentFileNames,
  )
...
async function saveAssembly() {
  ...
  try {
    let thumbnailId = extractThumbnailId(props.assembly.thumbnailUrl)

    if (itemFile.value !== null) {
      const newGuid = uuidv4()
      const presignedUrlResponse: AxiosResponse<string> = await apiservice.get<string>(
        urlservice.getGeneralAssemblyPresignedUrl(newGuid),
      )
...
<template>
  <BaseModal
    :is-open="isOpen"
    title="Rediger Generalforsamling"
    :is-loading="isLoading"
    @close="emit('close')"
  >
    <form @submit.prevent="saveAssembly">
      <!-- Title -->
      <div class="pb-4">
        <label for="edit_ga_title">Titel</label>
        <br />
        <input
          id="edit_ga_title"
      ...
      <div class="flex justify-between gap-4 pb-4">
        <div class="flex-1">
          <label for="edit_ga_section">Sektion</label>
          <br />
          <input
            id="edit_ga_section"
            class="w-full bg-theme-soft border border-theme-border rounded-xl p-2 text-theme-heading placeholder-theme-muted focus:outline-none focus:ring-2 focus:ring-theme-accent focus:border-theme-accent"
        ...
        <div class="flex-1">
          <label for="edit_ga_address">Adresse</label>
          <br />
          <input
            id="edit_ga_address"
            class="w-full bg-theme-soft border border-theme-border rounded-xl p-2 text-theme-heading placeholder-theme-muted focus:outline-none focus:ring-2 focus:ring-theme-accent focus:border-theme-accent"
        ...
        <div class="flex-1">
          <label for="edit_ga_date">Dato</label>
          <br />
          <input
            id="edit_ga_date"
      ...
      <!-- Description -->
      <div class="pb-4">
        <label for="edit_ga_description">Beskrivelse</label>
        <br />
        <textarea
      ...
      <div class="pb-4">
        <label for="edit_ga_file">Billede (valgfri — erstatter nuværende billede)</label>
        <br />
        <input
          id="edit_ga_file"
