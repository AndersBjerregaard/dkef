...
onMounted(async () => {
  ...
  if (!token.value) {
    error.value = 'Linket til ændring af e-mail adresse er ugyldigt.'
    success.value = false
  ...
  try {
    ...
    error.value = 'En fejl opstod ved bekræftelsen af e-mail adressen.'
  } finally {
    isLoading.value = false
  }
})
...
<template>
  <main>
    <div class="pt-24">
      <h2 class="mt-6 text-center text-3xl font-bold">Bekræft ny e-mail adresse</h2>
    </div>
        ...
        <div v-if="success">
          <div class="rounded-xl bg-green-900 bg-opacity-50 p-4 border border-green-700">
            <p class="text-sm text-green-200">
              Din e-mail adresse er nu ændret! Af sikkerhedsmæssige årsager er du blevet logget ud
              af alle dine aktive sessioner. Du kan nu logge ind med din nye e-mail adresse.
            </p>
          </div>
